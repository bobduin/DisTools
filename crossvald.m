...
function [err,cerr,nlabout] = crossvald(data,classf,n,kred,iter)
  ...
  if isdataset(data)
    ...
    prwaitbar(iter,s);
      ...
      prwaitbar(iter,j,[s num2str(j)]);
			eer{j} = feval(mfilename,data,classf,n,kred,1);
		end
    prwaitbar(0);
    ...
    if nargout == 0
      ...
      if ~iscell(classf)
        classf = {classf};
			...
			disperror(data,classf,mean(fe,3));
    else
      err = mean(fe,3);
    end
		return
    ...
    if numc > 1
      ...
      if numd > 1
        ...
        if numd > 1, prwaitbar(numd,jd,[snumd num2str(jd)]); end
				...
				rand('state',seed);
				[ee,cc,nn] = feval(mfilename,data{jd},classf{jc},n,kred,1);
				e(jd,jc) = ee;
				cerr(jd,jc) = {cc};
				nlabout(jd,jc) = {nn};
