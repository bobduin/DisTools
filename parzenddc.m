...
function [W,h] = parzenddc(d,h)
  ...
  if ~isempty(h)       % Take user settings for smoothing parameters.
    ...
    if all(diag(D(1:min(m,k),1:min(m,k))) == 0)
      % if diagonal 0 make it inf to avoid training set bias
      D(1:min(m,k),1:min(m,k)) = D(1:min(m,k),1:min(m,k)) + ...
                                 diag(inf*ones(1,min(m,k)));
    end
