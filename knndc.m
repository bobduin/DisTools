...
if nargin < 1 | isempty(d)
	W = prmapping(mfilename,'untrained',knn,par);
  if (isempty(knn))
    W = setname(W,'K-NND');
  else
    W = setname(W,[num2str(knn) '-NND']);
  end
  return; 
end
...
%[nlab,lablist,m,k,c,p,featlist] = prdataset(d);
...
if ~ismapping(knn)  % training (find knn)
	...
	W = prmapping(mfilename,'trained',{knn,JJ},lablist,k,c);
	W = setname(W,'KNNDC');
  ...
  [nn,nf,fl] = renumlab(classlist,featlist);
	if max(nf) > c
