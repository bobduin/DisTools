...
if nargin < 1 | isempty(d),
  W = prmapping(mfilename,{alf,pzero,prec});
...
if (isdataset(d) | isa(d,'double'))
  if ismapping(alf)
    ...
    % Store signature and spectrum in the USER field
    if isdataset(W),
      W = setname(W,['Projected ' updname(W.name)]);
      W = setsig(W,getdata(alf,'sig'));
      %W = setspec(W,getspec(alf));
      W = setuser(W,getspectrum(alf),'pe_spectrum');
    end
    return
  end
end
...
if ismapping(d)
  ...
  W = prmapping(mfilename,'trained',data,[],m,length(J));
  W = setname(W,'PE embedding');
  return
end
...
if pzero == 0,
  W = prmapping(mfilename,'trained',data,[],m,sum(sig));
else
  data.mdis = +d(:,pzero)';
  W = prmapping(mfilename,'trained',data,[],m,sum(sig));
end
