...
function [DT,DS,I,J,R] = genddat(varargin)

  % define defaults
  argin = setdefaults(varargin,[],[],[],[],'in');
  
  % define command as a fixed mapping
  if mapping_task(argin,'definition')
    DT = define_mapping(argin,'generator','Disdata sampling');
    return
  end
  
  % get parameters
  [D,M,K,L,type] = deal(argin{:});

	% If input is a cell array of datasets, run for all cells
	if (iscell(D)) % 
    [DT,DS,I,J,R] = runcells(mfilename,varargin{:});
    return
  end

  % D should be a square dismat referring to itself
  issquare(D); 
  
  % Generate trainset and testset by gendat
  if isempty(M)
    I = [1:size(D,1)]; 
    J = [];
  else
    [dummy,dumm,I,J] = gendat(D,M);
  end
  
  % Generate train part of repset
  if isempty(K), R = I; 
  elseif K == 0, R = [];
  else [dummy,dumm,R] = gendat(D(I,:),K); R = I(R);
  end
  
  % Add test part of repset
  if ~isempty(L) && ~isempty(J)
    [dummy,dumm,S,JJ] = gendat(D(J,:),L);
    R = [R; J(S)];
    if strcmp(type,'ex')
      % exclude repset from testset if desired
      J = J(JJ);
    end
  end
